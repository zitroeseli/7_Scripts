...
ds = driver.CreateDataSource('HOME/2_Netzwerk/activityNetwork1.shp')
...
for elements in edge:
	...
	# if no bicycle and no train (you find railway lines defined as busline, this is just to make sure, that nothing like rail or tram is included in activity network) 
	if 'rail' not in str(allow) and 'rail_electric' not in str(allow) and 'rail_urban' not in str(allow) and 'tram' not in str(allow) and 'bicycle' not in str(allow):
